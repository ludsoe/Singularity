...
function NDat.SendData(Data,Name,ply)
	...
	net.Start("Singularity_basenetmessage")
...
function NDat:InNetF(MSG,Data,ply)
	if MSG~=nil and NDat.NHook[MSG] then
...
net.Receive( "Singularity_basenetmessage", function( length, ply )
...
if(SERVER)then	
	...
	----------------------------------------------------]]--
	util.AddNetworkString( "Singularity_basenetmessage" )

	--Loops the players and prepares to send their data.
	function NDat.CyclePlayers()
		for nick, pdat in pairs( NDat.Data ) do
			local Max = 200
			if not IsValid(pdat.Ent) then continue end
			for id, Data in pairs( pdat.Data ) do
				if Max<=0 then continue end--We reached the maximum amount of data for this player.
				Max=Max-Data.Val
	...
	function NDat.AddData(Data,ply)
		if not Data.Dat then error("Netcode Failure! Missing Dat table. ["..tostring(Data.Name).."]") return end --Damn Missing Dat table....
