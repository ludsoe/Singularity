...
local Singularity = Singularity or {} --Localise the global table for speed.
...
local DTable = Singularity.Utl.DebugTable --Localise the debug storage.
local HTable = Singularity.Utl.Hooks --Localise the hook table for speed.
...
function Utl:RunHooks(Name,a1,a2,a3,a4,a5)--Run the HookHooks and return the most important return we get.
	...
	for I, H in pairs( Hook ) do --Loop all the HookHooks.
		xpcall(function()
			...
			--MsgAll(Name.." Hook Called! \n")
			if(R~=nil)then --Did we get a return?
				--MsgAll("Got a return! "..Name.." \n")
...
function Utl:MakeHook(Name) --Make the hookhook storage.
	if(not HTable[Name])then
		...
		hook.Add(Name,"SingularityHookMake",Func)
	else
