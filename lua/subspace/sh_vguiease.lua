...
Singularity.MenuCore = Singularity.MenuCore or {}
...
if(CLIENT)then
	...
	function MC.CreateFrame(Size,Visible,XButton,Draggable,CloseDelete)
		local Derma = vgui.Create( "DFrame" )
			if Derma then
				...
				function Derma:SetColor(Col) self.RenderColor = Col end Derma:SetColor(Singularity.GuiThemeColor.BG) --Dont Forget to set a color so we dont get nil Errors.
				function Derma:SetGradientColor(Col) self.RenderGradientColor = Col end Derma:SetGradientColor(Singularity.GuiThemeColor.GC) --Dont Forget to set a color so we dont get nil Errors.
				...
				Derma.Paint = function( self, w, h )
					...
					if GC.a>0 then
						surface.SetTexture( Singularity.GradientTex )
					...
					draw.DrawText(self.Title, "DermaDefault",tw+20,th, Singularity.GuiThemeColor.Text, 2)
				end		
			end
		return Derma
	end
	...
	function MC.CreatePanel(Parent,Size,Spot,Draw)
		local Derma = vgui.Create( "DPanel", Parent )
			...
			function Derma:SetColor(Col) self.RenderColor = Col end Derma:SetColor(Singularity.GuiThemeColor.BG) --Dont Forget to set a color so we dont get nil Errors.
			function Derma:SetGradientColor(Col) self.RenderGradientColor = Col end Derma:SetGradientColor(Singularity.GuiThemeColor.GC) --Dont Forget to set a color so we dont get nil Errors.
			function Derma:BasePaint(self, w, h)
				draw.RoundedBox( 16, 0, 0, w, h, self.RenderColor )
	...
	function MC.CreatePSheet(Parent,Size,Spot)
		local Derma = vgui.Create( "DPropertySheet", Parent )
			...
			function Derma:AddSheet(label, panel, material, NoStretchX, NoStretchY, Tooltip)
				...
				function Tab:SetColor(Col) self.RenderColor = Col end Tab:SetColor(Singularity.GuiThemeColor.FG)
			...
			function Derma:SetColor(Col) self.RenderColor = Col end Derma:SetColor(Singularity.GuiThemeColor.FG)
	...
	function MC.CreatePBar(Parent,Size,Spot,Progress)
		local Derma = vgui.Create( "DPanel", Parent )
			...
			function Derma:SetBGColor(Col) self.RenderBGColor = Col end Derma:SetBGColor(Singularity.GuiThemeColor.BG) --Dont Forget to set a color so we dont get nil Errors.
	...
	function MC.CreateText(Parent,Spot,Text,Color)
		local Derma = vgui.Create( "DLabel", Parent )
			...
			Derma:SetTextColor( Color or Singularity.GuiThemeColor.Text )
	...
	function MC.CreateAdvText(Parent,Size,Spot,Text,Color)	
		local Derma = vgui.Create( "DPanel", Parent )
			...
			function Derma:SetTextColor(Col) self.TextColor = Col end Derma:SetTextColor( Color or Singularity.GuiThemeColor.Text )
	...
	function MC.CreateSuperText(Parent,Spot,...)
		local args = { ... } -- Create a table of varargs
		local Derma = vgui.Create( "DPanel", Parent )
			Derma:SetPos( Spot.x, Spot.y )
			function Derma:SetText(Text) self.Text = Text end Derma:SetText( args or {""} )
			function Derma:SetAlpha(alpha) self.Alpha = alpha end Derma:SetAlpha(255)
			
			function Derma:Paint(w,h)
				local Color = Color(255,255,255,255)
				local Off = 5
				surface.SetFont( "Default" )
				for _, varg in pairs(self.Text) do
					local Type = type(varg)
					if Type=="table" then
						Color = varg
					elseif Type == "function" then
						local Retu = varg()
						local Type2 = type(Retu)
						if Type2 == "table" then
							Color = varg
						elseif Type2 == "string" then
							local Alpha = 255 if type(self.Alpha) == "function" then Alpha = self.Alpha(self) else Alpha = self.Alpha end
							surface.SetTextColor( Color.r, Color.g, Color.b, Alpha )
							surface.SetTextPos( Off, 0 )
							tw,th = surface.GetTextSize(varg)
							surface.DrawText(varg)
							Off=Off+tw
						end
				...
				Derma.Think = function(self) 	
					if self:GetSelected() and self:GetSelected()[1] then 
						...
						if selectedValue ~= self.Selected then 
							...
							Func(selectedValue,self:GetSelected())
						end
					end 
					self:OldThink() 
				end 
	...
	function MC.CreateButton(Parent,Size,Spot,Text,OnClick)
		local Derma = vgui.Create( "DButton", Parent )
			...
			Derma:SetColor(Singularity.GuiThemeColor.Text)
			
			function Derma:SetBGColor(Col) self.RenderColor = Col end Derma:SetBGColor(Singularity.GuiThemeColor.FG)
			function Derma:SetGradientColor(Col) self.RenderGradientColor = Col end Derma:SetGradientColor(Singularity.GuiThemeColor.GC)
			function Derma:SetGradientColorOver(Col) self.RenderGradientColorOver = Col end Derma:SetGradientColorOver(Singularity.GuiThemeColor.GHO)
